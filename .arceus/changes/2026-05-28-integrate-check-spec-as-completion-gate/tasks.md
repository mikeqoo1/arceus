...
- [x] T-8 2000 字警告：stderr 印 + audit 檔開頭插 `> [!WARNING]` 區塊；退出碼不變
...
- [x] T-20 2000 字警告：fake binary `oversize` mode 觸發 + assert `isReportOversize(result.report) === true`（CLI 層的 stderr 警告由現有 e2e dogfood 證實）
